...
class ConfigurationBuilder {
	...
	build() {
		...
		return ConfigurationBuilder.mergeCliArgs( configuration, this.argv )
			...
			.then( () => this.mergeUserRcFile( configuration ) )
	...
	mergeUserRcFile( configuration ) {
		return Promise.try( () => {
			const homedir    = os.homedir();
			const userRcPath = path.resolve( homedir, ConfigurationBuilder.RC_FILENAME );

			this.log.debug( `Looking for user RC file at '${userRcPath}'…` );
			const rcFile = require( userRcPath );
		} )
			.catch( error => this.log.debug( "No RC file found. Ignoring" ) )
			.return( configuration );
	}
}
