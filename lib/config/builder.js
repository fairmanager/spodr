...
class ConfigurationBuilder {
	...
	/**
	 * @returns {String} The name of the file in which version locks are stored.
	 */
	static get LOCKFILE_FILENAME() {
		return ".spodrlock.json";
	}
	...
	mergeRepositories( from, to ) {
		_.forEach( from, repository => {
			...
			const propertiesToCheck = Object.keys( repository );
			Object.keys( propertiesToCheck ).forEach( property => {
				if( !Object.keys( existingRepositoryConfiguration ).includes( propertiesToCheck[ property ] ) ) {
					// eslint-disable-next-line no-console
					this.log.warn( `Property '${propertiesToCheck[ property ]}' in ${repository.name} is unknown.` );
				}
			} );
			...
			.map( directory => {
				...
				return fs.statAsync( path.join( directory, "package.json" ) )
					...
					.catch( () => {
						...
						repository.install = false;
					} )
					.return( repository );
			} )
			.then( repositories => {
				this.mergeRepositories( repositories, configuration.repositories );
				return configuration;
			} );
	}
	...
	static mergeConfigurationFile( configuration, file ) {
		try {
			...
			this.mergeRepositories( configurationFile.repositories, configuration.repositories );
		...
		return ConfigurationBuilder.getDirectories( root )
			.map( directory => {
				try {
					...
					return this.mergeRepositories( [ rcFile ], configuration.repositories );
	...
	static mergeCliArgs( configuration, argv ) {
		return Promise.try( () => {
			...
			if( args.upstream ) {
				configuration.targetRemote = "upstream";
			}
			if( args.remote ) {
				configuration.targetRemote = args.remote;
			}

			if( args.master ) {
				configuration.targetBranch = "master";
			}
			if( args.branch ) {
				configuration.targetBranch = args.branch;
			}

			if( "pin-roots" in args ) {
				configuration.packageManagement.pinRoots = args[ "pin-roots" ];
			}
			if( args.update ) {
				configuration.packageManagement.updateDependencies = true;
			}
