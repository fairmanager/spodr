							...
							for( const moduleHost of relevantHostVersions ) {
								for( const modulePlugin of relevantPlugins ) {
									if( moduleHost.hasBranchFor( modulePlugin.name ) ) {
										const dependency = moduleHost.dependencyByName( modulePlugin.name );
										if( dependency.version !== modulePlugin.version ) {
											log.warn( `Peering conflict: '${hostPackageName}@${moduleHost.version}' already depends on '${modulePlugin.name}' in version '${dependency.version}'. Can't peer with '${modulePlugin.name}@${modulePlugin.version}'. Lock '${modulePlugin.name}' to either '${dependency.version}' or '${modulePlugin.version}' to resolve.` );
										}
										continue;
									}
