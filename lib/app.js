...
class Application {
	...
	update() {
		...
		if( skipGitUpdate ) {
			updateTasks = Promise.resolve( this.repositories );
		} else {
			updateTasks = tasks.update( this.config );
		}
	...
	prepareRepositories() {
		...
		repositories.sort( ( a, b ) => {
			...
			const getAbsoluteSortKey = ( element, depth = 0 ) => {
				if( getSortKey( element ) === element.name ) {
					return element.name;
				}
				if( depth > 10 ) {
					throw new Error( "At least two repositories will be sorted indefinitely with the current configuration." );
				}
				return getAbsoluteSortKey( this.config.repositories.find( repository => repository.name === getSortKey( element ) ), depth + 1 );
			};
			const getSortDirection = ( elementA, elementB ) => {
				if( elementA.sortAbove && elementB.sortAbove ) {
					return 0;
				} else if( elementA.sortBelow && elementB.sortBelow ) {
					return 0;
				} else if( elementA.sortBelow || elementB.sortAbove ) {
					return 1;
				} else if( elementA.sortAbove || elementB.sortBelow ) {
					return 1;
				}
				return 0;
			};
			const valueA = getAbsoluteSortKey( a );
			const valueB = getAbsoluteSortKey( b );
			if( valueA === valueB ) {
				return getSortDirection( a, b );
			}
