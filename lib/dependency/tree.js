...
class DependencyTree {
	 ...
	 */
	constructor( configuration, withDevDependencies ) {
		...
		this.versionResolver = new VersionResolver( configuration );
	...
	/**
	 * Find all packages in the aggregate cache whose name matches a given pattern.
	 * @param {RegExp} name A regular expression to match against the package names.
	 * @return {Array<String>} The names of the matching packages.
	 */
	findMatchingPackagesInAggregateCache( name ) {
		const matches = [];
		for( const packageName of this.aggregateCache.keys() ) {
			if( name.test( packageName ) ) {
	...
	/**
	 * Regardless of the requested version, always use the given version in the dependency tree.
	 * This is usually used for the root projects; dependency versions are usually controlled
	 * through the version locking mechanism.
	 * @param {String} packageName The name of the package.
	 * @param {String} versionToUse The version the package should be pinned to.
	 */
	 ...
	 * @return {DependencyTree}
	 */
	static fromPackagePaths( configuration, paths, withDevDependencies = true, pinRoots = false ) {
		const dependencyTree = new DependencyTree( configuration, withDevDependencies );
	 ...
	 * @param {Boolen} [withDevDependencies=true] Should devDependencies be considered for this tree?
	 */
	static fromStorageRoot( configuration, storageRoot, withDevDependencies = false ) {
		log.notice( `Generating dependency tree from '${storageRoot}'…` );
		const statCacheHelper = new DependencyTree( configuration );
		...
		return fs.readdirAsync( storageRoot )
			...
			.then( packagePaths => DependencyTree.fromPackagePaths( configuration, packagePaths, withDevDependencies ) )
			.then( dependencyTree => {
