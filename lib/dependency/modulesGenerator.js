...
class ModulesGenerator {
	...
	__clearNodeModules( moduleParent ) {
		...
		log.debug( `Clearing ${moduleParent}...` );
		return rimrafAsync( nodeModulesPath, {
			...
			.catch( {
				...
				const delay = Math.round( 100 + Math.random() * 300 );
				log.warn( `${error.message} - Retrying in ${delay}ms. Ctrl+C to abort.` );
				return Promise.delay( delay )
					.then( () => this.__clearNodeModules( moduleParent ) );
			} )
			.return( null );
	}
		...
		return Promise.each( moduleParent.branches, dependency => {
			if( dependency.package.bin ) {
				...
				if( typeof dependency.package.bin === "string" ) {
					const unscopedDependencyName = dependency.name.replace( /^@.+\//, "" );
					return this.__createLinksForDependency( dependency, moduleParent.storageLocation, unscopedDependencyName, dependency.package.bin )
